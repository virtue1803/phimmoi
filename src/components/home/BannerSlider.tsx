...
import { useTrailerLauncher, useVideos } from "@/hooks/useVideos";
import { getImageUrl } from "@/lib/tmdb";
import { MediaBase } from "@/types/tmdb";
import { getTitle } from "@/utils/format";
import { OUTLINE_BUTTON, PRIMARY_BUTTON } from "@/utils/styles";
...
export default function BannerSlider({ items }: BannerSliderProps) {
  ...
  const title = active ? getTitle(active) : "";
  const { playTrailer } = useTrailerLauncher(videos?.results, title);
    ...
    >
      ...
      >
        <div className="w-full text-center lg:w-3/5 lg:text-left">
          <h1
            style={{ animation: "fadeSlideDown 300ms ease-in-out both" }}
            className="text-3xl font-extrabold leading-tight text-white sm:text-4xl lg:text-6xl"
          >
            {title}
          </h1>
          ...
          >
            <Link href={`/movies/${active.id}`} className={PRIMARY_BUTTON}>
              Watch now
            </Link>
            ...
            <button
              ...
              className={`flex items-center gap-2 disabled:opacity-60 ${OUTLINE_BUTTON}`}
            >
